...
def export_to_csv(modeladmin, request, queryset):
    ...
    response['Content-Disposition'] = 'attachment;filename={}.csv'.format(opts.verbos_name)
    writer = csv.writer(response)

    fields = [field for field in opts.get_fields() if not field.many_to_many and not field.one_to_many]
    # csv 파일 컬럼 타이틀
    writer.writerow(([field.varbose_name for field in fields]))
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ['id', 'first_name', 'last_name', 'email', 'address', 'paid', order_detail, order_pdf, 'created', 'updated']
